function plot_and_save(meas_dict, model_output, name_for_plot, measurement_folder)
    ...
    potential_IDs_coordinate_data = py.BEASY_IN_OUT.xyz_coordinates_for_Mesh_point_IDS(position_ID_1, fullfile(measurement_folder,'BU_Jacket_newCurves.dat'));
    ...
    legend(ax1, {'Measurement data','Model Output'}, 'Location','southeast');
    
    legend(ax2, {'Measurement data','Model Output'}, 'Location','southeast');
    
    axes('position',[.25 .35 .15 .15], 'NextPlot', 'add')
    plot(sorted_position_ID_2(2:3), meas_current_density(2:3));
    plot(sorted_position_ID_2(2:3), output_current_density(2:3));
    %hold on;
